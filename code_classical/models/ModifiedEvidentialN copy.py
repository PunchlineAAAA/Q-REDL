...
class ModifiedEvidentialNet(nn.Module):
    ...
    #     # 将复数转换为实数evidence
    #     real_logits = self.complex_to_real(complex_logits)
        
    #     # 应用softplus获取evidence
    #     evidence = F.softplus(real_logits)
        
    #     # 计算Dirichlet参数 α = evidence + λ
    #     alpha = evidence + self.lamb2

    #     if compute_loss:
    #         # 标准的one-hot编码
    #         labels_1hot = torch.zeros_like(evidence).scatter_(-1, labels_.unsqueeze(-1), 1)
            
    #         # 计算projected probability (R-EDL的核心)
    #         # P_X(x) = α_X(x) / S_X，其中 S_X = Σα_X(x)
    #         alpha_sum = torch.sum(alpha, dim=-1, keepdim=True)
    #         projected_prob = alpha / alpha_sum
            
    #         # R-EDL损失：直接优化projected probability与one-hot标签的MSE
    #         # L_redl = (1/|D|) * Σ Σ (y[x] - P_X(x))²
    #         self.loss_mse = F.mse_loss(projected_prob, labels_1hot)
    ...
    #     elif return_output == 'soft':
    #         alpha_sum = torch.sum(alpha, dim=-1, keepdim=True)
    #         return alpha / alpha_sum  # 返回projected probability
    #     elif return_output == 'alpha':
    #         return alpha
    #     else:
    #         raise AssertionError
